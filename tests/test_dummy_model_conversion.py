...
GGUF_AVAILABLE = is_module_available("llama_cpp_python") or is_module_available("llama_cpp_python")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if export_format in ["onnx", "torchscript"]:
                pytest.skip(f"{export_format.upper()} export failed for config {model_cfg}: {e}")
            else:
                raise
