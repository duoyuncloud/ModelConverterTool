    ...
    def convert(
        ...
        try:
            ...
            if not os.path.exists(input_source) and not input_source.startswith("hf:"):
                logger.info(
                    f"Input source {input_source} not found locally, attempting to download from HuggingFace Hub..."
                )
                ...
                try:
                    AutoModel.from_pretrained(input_source, cache_dir=cache_dir).save_pretrained(
                        cache_dir, safe_serialization=False
                    )
            ...
            if output_format == "hf":
                ...
                success = self._convert_to_safetensors(model_name, output_path, model_type, device, offline_mode)
...
"""
    ...
    def _convert_to_torchscript(
        ...
        try:
            ...
            # Save HF format files
            self._save_hf_format_files(model_name, torchscript_file.parent, tokenizer, config, "torchscript")

            # Create model card
            self._create_model_card(torchscript_file.parent, model_name, "torchscript", model_type)
