...
def model_provider(
    ...
    if args.use_mcore_models:
        # Handle missing ammo attributes for conversion scripts
        ammo_load_classic_megatron_to_mcore = getattr(args, "ammo_load_classic_megatron_to_mcore", False)
        ammo_convert_te_to_local_spec = getattr(args, "ammo_convert_te_to_local_spec", False)

        if ammo_load_classic_megatron_to_mcore:
            model._register_load_state_dict_pre_hook(mcore_gpt_load_classic_state_dict_pre_hook)
        elif ammo_convert_te_to_local_spec:
