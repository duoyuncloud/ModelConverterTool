...
def save_checkpoint_and_time(
    iteration, model, optimizer, opt_param_scheduler, num_floating_point_operations_so_far, checkpointing_context
):
    args = get_args()
    timers = get_timers()
...
def train(
    ...
    if args.overlap_param_gather and args.delay_param_gather:
        config.param_sync_func = [
            lambda x: optimizer.finish_param_sync(model_index, x) for model_index in range(len(model))
        ]
        if len(model) == 1:
    ...
    if args.log_straggler:
        ...
        stimer.configure(
            world, rank, mmcnt=mmcnt, enabled=not args.disable_straggler_on_startup, port=args.straggler_ctrlr_port
        )
    total_flops = 0.0
    ...
    while iteration < args.train_iters:
        if args.profile and iteration == args.profile_step_start and torch.distributed.get_rank() in args.profile_ranks:
        ...
        if (
            ...
            if args.use_distributed_optimizer and args.overlap_param_gather:
                optimizer.disable_pre_hook()
            assert check_param_hashes_across_dp_replicas(model), "Parameter hashes not matching across DP replicas"
            torch.distributed.barrier()
        ...
        if args.profile and iteration == args.profile_step_end and torch.distributed.get_rank() in args.profile_ranks:
