...
def compute_weight_and_optimizer_memory(args, verbose=False):
    ...
    num_parameters_in_transformer_layers = (
        ...
        * (
            # Attention.
            ((1 + (args.num_query_groups / args.num_attention_heads)) * query_projection_to_hidden_size_ratio)
            # MLP.
    ...
    num_bytes_per_parameter = 18 if not args.use_distributed_optimizer else 6 + (12 / args.data_parallel_size)
...
def compute_activation_memory(args, num_microbatches, verbose=False):
    ...
    activation_memory += args.seq_length * args.micro_batch_size * args.hidden_size * args.pipeline_model_parallel_size
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        ...
        in_flight_microbatches = math.ceil(interleaved_schedule_memory_penalty * args.pipeline_model_parallel_size)
        if verbose:
...
def report_theoretical_memory(args, num_microbatches=None, verbose=False):
    weight_and_optimizer_memory = compute_weight_and_optimizer_memory(args, verbose=verbose) / NUM_BYTES_IN_MEGABYTE
    ...
    activation_memory = (
        compute_activation_memory(args, num_microbatches=num_microbatches, verbose=verbose) / NUM_BYTES_IN_MEGABYTE
    )
