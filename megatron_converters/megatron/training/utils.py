...
def calc_params_l2_norm(model):
    ...
    if mpu.get_expert_model_parallel_world_size() == 1:
        ...
        torch.distributed.all_reduce(
            norm_2, op=torch.distributed.ReduceOp.SUM, group=mpu.get_tensor_and_expert_parallel_group()
        )
        torch.distributed.all_reduce(
...
def append_to_progress_log(string, barrier=True):
    ...
    if torch.distributed.get_rank() == 0:
        with open(progress_log_filename, "a") as f:
            ...
            f.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\tJob ID: {job_id}\t" f"# GPUs: {num_gpus}\t{string}\n"
            )
