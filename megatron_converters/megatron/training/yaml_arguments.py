...
str_dtype_to_torch = {"float32": torch.float32, "float16": torch.float16, "bfloat16": torch.bfloat16}
...
def validate_yaml(args, defaults={}):
    ...
    # Checks.
    model_parallel_size = (
        args.model_parallel.pipeline_model_parallel_size * args.model_parallel.tensor_model_parallel_size
    )
    ...
    if args.model_parallel.pipeline_model_parallel_size > 1:
        if args.model_parallel.pipeline_model_parallel_split_rank is not None:
            assert (
                args.model_parallel.pipeline_model_parallel_split_rank
                < args.model_parallel.pipeline_model_parallel_size
    ...
    if args.num_layers_per_virtual_pipeline_stage is not None:
        ...
        assert (
            args.language_model.num_layers % args.model_parallel.transformer_pipeline_model_parallel_size == 0
    ...
    if args.model_parallel.expert_model_parallel_size > 1:
        ...
        assert (
            args.language_model.num_moe_experts % args.model_parallel.expert_model_parallel_size == 0
        ), "Number of experts should be a multiple of expert model parallel_size."
        assert not args.model_parallel.fp16, "Expert parallelism is not supported with fp16 training."
