...
def validate_args(args, defaults={}):
    ...
    if args.overlap_param_gather:
        ...
        assert args.overlap_grad_reduce, "--overlap-grad-reduce should be turned on when using --overlap-param-gather"
    ...
    if args.retro_add_retriever:
        ...
        assert args.train_samples is not None, "args.train_samples should be auto-loaded from the retro config."

        # Sequence parallelism unsupported.
        assert not args.sequence_parallel, "retro currently does not support sequence parallelism."

        # Pipeline parallelism unsupported.
        assert args.pipeline_model_parallel_size == 1, "retro currently does not support pipeline parallelism."
    ...
    if args.num_experts is not None:
        ...
        if args.tensor_model_parallel_size > 1:
            assert args.sequence_parallel, "When using MoE and tensor parallelism, sequence parallelism must be used."
...
def core_vit_transformer_config_from_args(args, config_class=None):
    ...
    for f in dataclasses.fields(config_class):
        if hasattr(args, f.name):
            kw_args[f.name] = getattr(args, f.name)
    kw_args["num_layers"] = args.vit_num_layers
    kw_args["hidden_size"] = args.vit_hidden_size
    kw_args["ffn_hidden_size"] = args.vit_ffn_hidden_size
    kw_args["num_attention_heads"] = args.vit_num_attention_heads
    kw_args["kv_channels"] = args.vit_hidden_size // args.vit_num_attention_heads
    kw_args["add_bias_linear"] = args.vit_add_bias_linear
    kw_args["add_qkv_bias"] = args.vit_add_qkv_bias
    # kw_args['persist_layer_norm'] = not args.no_persist_layer_norm
    # kw_args['layernorm_zero_centered_gamma'] = args.apply_layernorm_1p
    kw_args["normalization"] = args.vit_normalization
    kw_args["layernorm_epsilon"] = args.vit_norm_epsilon
    # kw_args['deallocate_pipeline_outputs'] = True
    kw_args["pipeline_dtype"] = args.params_dtype
    # kw_args['batch_p2p_comm'] = not args.overlap_p2p_comm
    # kw_args['num_moe_experts'] = args.num_experts
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument(
        "--fp8-interval", type=int, default=1, help="Scaling update interval for fp8", dest="fp8_interval"
    )
    group.add_argument(
...
def _add_retro_args(parser):
    ...
    group.add_argument(
        "--retro-encoder-layers", type=int, default=2, help="Number of layers to use for the retrieval " "encoder."
    )
    group.add_argument(
        "--retro-encoder-hidden-dropout", type=float, default=0.1, help="Hidden dropout for " "retrieval encoder."
    )
    group.add_argument(
        "--retro-encoder-attention-dropout", type=float, default=0.1, help="Attention dropout for " "retrieval encoder."
    )
    group.add_argument(
        "--retro-num-neighbors", type=int, default=2, help="Number of neighbors to retrieve during " "pretraining."
    )
    group.add_argument(
    ...
    for action in group._group_actions:
        ...
        assert (
            prefix == "retro"
        ), "Retro args must be prefixed with '--retro-*', for consistent " "styling. Please fix '%s'." % ", ".join(
...
def _add_network_size_args(parser):
    ...
    group.add_argument("--query-rope-channels", type=int, default=None, help="query_rope_dim in MLA")
    group.add_argument("--query-nope-channels", type=int, default=None, help="query_nope_dim in MLA")
    group.add_argument("--query-lora-hidden-size", type=int, default=None, help="query_lora hidden size in MLA")
    group.add_argument("--kv-lora-hidden-size", type=int, default=None, help="kv_lora hidden size in MLA")
    group.add_argument(
        "--kv-channels",
        type=int,
        default=None,
    ...
    group.add_argument(
        "--rotary-percent", type=float, default=1.0, help="Percent of rotary dimension to use, default 100%%"
    )
    group.add_argument("--rotary-base", type=float, default=10000, help="base of rotary position embedding")
    group.add_argument("--use-longrope", action="store_true", help="whether use longrope")
    group.add_argument(
        "--longrope-short-factor", nargs="*", default=None, type=float, help="short factor used in longrope"
    )
    group.add_argument(
        "--longrope-long-factor", nargs="*", default=None, type=float, help="long factor used in longrope"
    )
    ...
    group.add_argument("--layernorm-epsilon", type=float, default=1e-5, help="Epsilon for layer norm and RMS norm.")
    group.add_argument(
        "--apply-layernorm-1p",
        action="store_true",
    ...
    group.add_argument(
        "--squared-relu", action="store_true", help="Use squared relu activation instead of default gelu"
    )
    group.add_argument(
        "--swiglu", action="store_true", help="Use gated linear units and SiLU activation instead of default gelu"
    )
    group.add_argument(
    ...
    group.add_argument(
        "--untie-embeddings-and-output-weights", action="store_true", help="Untie embeddings and output weights."
    ),
    group.add_argument("--use-mla", action="store_true", help="Whether use mla.")
    ...
    group.add_argument(
        "--mup-base-hidden-size", type=int, default=256, help="hidden size of small model used in mu-transfer"
    )
    return parser
...
def _add_logging_args(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default="hwinf_dcm",
        help="The one-logger username or team name. Will ignore if " "--enable-one-logger is not set",
    )
    group.add_argument(
        "--one-logger-run-name",
        type=str,
...
def _add_training_args(parser):
    ...
    group.add_argument(
        "--tp-comm-overlap-cfg", type=str, default=None, help="Config file when tp_comm_overlap is enabled."
    )
    group.add_argument(
    ...
    # multimodal
    group.add_argument("--tune-vision", action="store_true", default=False)
    group.add_argument("--tune-resampler", action="store_true", default=False)
    group.add_argument("--tune-llm", action="store_true", default=False)
    ...
    group.add_argument(
        "--log-task-loss-interval", type=int, default=100, help="Report multi task loss and timing interval."
    )
    group.add_argument(
        "--exit-interval",
        type=int,
    ...
    group.add_argument(
        "--disable-bias-linear", action="store_false", help="Disable bias in the linear layers", dest="add_bias_linear"
    )
    group.add_argument(
        "--add-qkv-bias", action="store_true", help="Enable bias only in the QKV linear layers", dest="add_qkv_bias"
    )
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default="linear",
        choices=["constant", "linear", "cosine", "inverse-square-root", "warmup-stable-exponential"],
    ...
    group.add_argument(
        ...
        type=float,
        default=0.5,
        help="drop rate for warmup-stable-exponential, "
        "exp: lr * (exp_drop_rate ** ((num_step - (train_steps - decay_steps) / drop_cycle_steps)))",
    )
    group.add_argument(
        "--wsd-drop-cycle-iters",
        type=float,
        default=0.3,
        help="exponential drop cycle iters for warmup-stable-exponential, "
        "when drop_cycle_iters <= 1.0, it is interpreted as a fraction of lr_decay_iters, "
        "exp: lr * (exp_drop_rate ** ((num_step - (train_steps - decay_steps) / drop_cycle_steps)))",
    )
    group.add_argument(
        "--wsd-drop-cycle-samples",
        type=float,
        default=0.3,
        help="exponential drop cycle samples for warmup-stable-exponential, "
        "when drop_cycle_samples <= 1.0, it is interpreted as a fraction of lr_decay_samples, "
        "exp: lr * (exp_drop_rate ** ((num_step - (train_steps - decay_steps) / drop_cycle_steps)))",
    )
    group.add_argument(
        "--lr-warmup-fraction",
    ...
    group.add_argument(
        "--decoupled-lr", type=float, default=None, help="Separate learning rate for the input and output layer"
    )
    group.add_argument(
        "--decoupled-min-lr",
        type=float,
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument("--merge-mode", type=str, default="megatron_to_pt", choices=["megatron_to_pt", "pt_to_hf"])
    return parser
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        "--overlap-grad-reduce", action="store_true", default=False, help="If set, overlap DDP grad reduce."
    )
    group.add_argument(
...
def _add_data_args(parser):
    ...
    group.add_argument(
        "--no-mmap-bin-files", action="store_false", help="Disable mmap-ing of .bin files.", dest="mmap_bin_files"
    )
    group.add_argument(
        "--mock-data",
        action="store_true",
    ...
    group.add_argument("--use_modelbest_sdk", action="store_true", help="use training datasets with sstable format")
    group.add_argument(
        "--clear_sampler_state", action="store_true", help="whether clear the max error state of megatron data sampler"
    )
    ...
    group.add_argument(
        ...
        choices=[
            ...
            "AutoTokenizer",
            "MiniCPMV2Tokenizer",
            "MiniCPMV2P5Tokenizer",
            "MiniCPMV2P6Tokenizer",
            "MiniCPM3OTokenizer",
            "NullTokenizer",
        ],
    ...
    group.add_argument("--custom-tokenizer-eod", type=int, default=None, help="custom tokenizer eod.")
    group.add_argument(
    ...
    group.add_argument("--use-packed-seq", action="store_true", help="Use span-based attn.")
    ...
    group.add_argument("--use_best_fit", help="whether keep every data intact", action="store_true")

    # multimodal dataset
    group.add_argument("--use-datapipe", action="store_true", default=False)
    group.add_argument(
        "--item-builder-type",
        type=str,
        default="llama3_chat",
        choices=["qwen2_chat", "llama3_chat", "minicpm3_chat"],
        help="pretraining objectives",
    )
    group.add_argument("--skip-overlength", action="store_true", default=False, help="is skip over length data")
    group.add_argument("--skip-no-image", action="store_true", default=False, help="is skip no image data")
    group.add_argument("--img-aug", action="store_true", default=False, help="use image aug")
    group.add_argument("--min-resolution", default=10, type=int, help="min resolution to filter out images")
    group.add_argument("--no-system-prompt", action="store_true", default=False)
    group.add_argument("--aug-size", action="store_true", default=False)
    group.add_argument(
        "--use-image-id",
        action="store_true",
        default=False,
        help="If set, use special token <image_id> </image_id> in prompt.",
    )
    group.add_argument("--use-mm-new-schema", action="store_true", default=False)
    group.add_argument("--total-max-length", default=4096, type=int, help="total max length of input")
    group.add_argument("--skip-files", default=0, type=int)
    group.add_argument("--force-batch", action="store_true", default=False)
...
def _add_vision_args(parser):
    ...
    group.add_argument("--vit-num-layers", type=int, default=None, help="Number of ViT transformer layers.")
    group.add_argument("--vit-hidden-size", type=int, default=None, help="ViT Tansformer hidden size.")
    group.add_argument(
        "--vit-ffn-hidden-size",
        type=int,
        default=None,
        help="ViT Transformer Feed-Forward Network hidden size. " "This is set to 4*hidden-size if not provided",
    )
    group.add_argument(
        "--vit-num-attention-heads", type=int, default=None, help="Number of transformer attention heads."
    )
    group.add_argument(
        "--vit-normalization",
        default="LayerNorm",
    ...
    group.add_argument("--vit-norm-epsilon", type=float, default=1e-6, help="Epsilon for ViT layer norm and RMS norm.")
    group.add_argument(
        "--vit-disable-bias-linear",
        action="store_false",
        help="Disable bias in the linear layers",
        dest="vit_add_bias_linear",
    )
    group.add_argument("--vit-add-qkv-bias", action="store_true", help="Enable bias only in the QKV linear layers")
    group.add_argument("--vision-encoder", type=str, default="clip")
    group.add_argument("--drop-vision-last-layer", action="store_true", help="is drop last layer")
    group.add_argument("--img-size", default=224, type=int, help="images input size for vision encoder")
    # for adaptive resolution + slice
    group.add_argument("--use-adaptive-slice", action="store_true", default=False)
    group.add_argument("--use-dynamic-batch", action="store_true", default=False)
    group.add_argument("--max-slice-nums", default=9, type=int)
    group.add_argument("--patch-size", default=14, type=int)
    group.add_argument("--scale-resolution", default=448, type=float)
    group.add_argument("--query-num", default=64, type=int, help="query numbers")
    ...
    group.add_argument(
        "--qk-layernorm", action="store_true", help="Whether to layer normalize the q and k attention embeddings."
    )
...
def _add_moe_args(parser):
    ...
    group.add_argument(
        "--moe-router-topk", type=int, default=2, help="Number of experts to route to for each token. The default is 2."
    )
    group.add_argument(
        "--moe-grouped-gemm",
        action="store_true",
    ...
    group.add_argument(
        "--moe-token-dispatcher-type", type=str, choices=["allgather", "alltoall"], default="allgather", help="."
    )
    group.add_argument(
