...
def get_tensor_model_parallel_group(check_initialized=True):
    ...
    if check_initialized:
        if _TENSOR_MODEL_PARALLEL_GROUP is None:
            # For conversion purposes, return None instead of asserting
            print("[DEBUG] tensor model parallel group is not initialized, returning None for conversion")
            return None
...
def get_pipeline_model_parallel_group():
    ...
    if _PIPELINE_MODEL_PARALLEL_GROUP is None:
        # For conversion purposes, return None instead of asserting
        print("[DEBUG] pipeline model parallel group is not initialized, returning None for conversion")
        return None
...
def get_tensor_and_expert_parallel_group():
    assert _TENSOR_AND_EXPERT_PARALLEL_GROUP is not None, "tensor and expert parallel group is not initialized"
    return _TENSOR_AND_EXPERT_PARALLEL_GROUP


def get_data_modulo_expert_parallel_group():
    assert _DATA_MODULO_EXPERT_PARALLEL_GROUP is not None, "data modulo expert parallel group is not initialized"
...
def get_tensor_model_parallel_rank():
    ...
    if group is None:
        # For conversion purposes, return 0 when group is not initialized
        print("[DEBUG] tensor model parallel group is None, returning rank 0 for conversion")
        return 0
...
def get_pipeline_model_parallel_rank():
    ...
    if group is None:
        # For conversion purposes, return 0 when group is not initialized
        print("[DEBUG] pipeline model parallel group is None, returning rank 0 for conversion")
        return 0
    return torch.distributed.get_rank(group=group)
...
def get_data_parallel_rank(with_context_parallel=False):
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        return torch.distributed.get_rank(group=get_data_parallel_group(with_context_parallel=with_context_parallel))
    else:
        return 0
...
def get_expert_model_parallel_rank():
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        tensor_and_expert_parallel_rank = torch.distributed.get_rank(group=get_tensor_and_expert_parallel_group())
