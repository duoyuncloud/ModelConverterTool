...
def unpermute(
    ...
    """
    if padded_mode:
        return unpermute_with_padded_tokens(permuted_tokens, sorted_indices, probs, restore_shape=restore_shape)
...
def unpermute_with_padded_tokens(
    ...
    assert permuted_tokens.shape == indices.shape, "Shape mismatch between permuted_tokens and indices."
...
def topk_softmax_with_capacity(
    ...
    if capacity_factor is None:
        ...
        if pad_to_capacity:
            ...
            final_indices = top_indices.clone().masked_fill_(exceed_mask, torch.iinfo(torch.long).max)
            tokens_per_expert_before_capacity = topk_mask.sum(dim=0)
...
def save_to_aux_losses_tracker(name: str, loss: torch.Tensor, layer_number: int, num_layers: int):
    ...
    if name not in parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER:
        parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER[name] = torch.zeros(num_layers, device=loss.device)
