...
# For conversion scripts, we don't need transformer_engine
te = None
...
_te_version = packaging.version.Version("0.0.0")  # Dummy version for conversion scripts
...
class TENorm:
    ...
    def __new__(
        ...
        # For conversion scripts, return None instead of TE layers
        return None


class TELinear(torch.nn.Module):  # Fallback to torch.nn.Module for conversion scripts
    """
...
class TELayerNormColumnParallelLinear(torch.nn.Module):  # Fallback for conversion scripts
    """
    ...
    def __init__(
        ...
        *,
        ...
        if _te_version >= packaging.version.Version("0.11.0"):
            ...
            raise ValueError(f"Transformer Engine v{_te_version} does not support {self.config.normalization}.")
...
class TEDotProductAttention(torch.nn.Module):  # Fallback for conversion scripts
    """
    ...
    def forward(
        ...
        packed_seq_kwargs = dataclasses.asdict(packed_seq_params) if packed_seq_params is not None else {}
...
class TEDelayedScaling:  # Fallback for conversion scripts
    """
