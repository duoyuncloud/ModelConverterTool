...
class T5MaskedWordPieceDataset(MaskedWordPieceDataset):
    ...
    def _key_config_attributes() -> List[str]:
        ...
        """
        return super(T5MaskedWordPieceDataset, T5MaskedWordPieceDataset)._key_config_attributes() + [
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[int, numpy.ndarray]]:
        ...
        (
            ...
            _,
            masked_spans,
        ) = self._create_masked_lm_predictions(tokens, target_sequence_length, numpy_random_state)
        ...
        encoder_input = numpy.pad(encoder_input, (0, length_pads_encoder), constant_values=self.config.tokenizer.pad)

        decoder_input = numpy.array(decoder_input, dtype=numpy.int64)
        decoder_input = numpy.pad(decoder_input, (0, length_pads_decoder), constant_values=self.config.tokenizer.pad)
    ...
    @staticmethod
    def _make_attention_mask(source_block: numpy.ndarray, target_block: numpy.ndarray) -> numpy.ndarray:
