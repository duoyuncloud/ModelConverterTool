...
class MaskedWordPieceDataset(MegatronDataset):
    ...
    @staticmethod
    def build_low_level_dataset(dataset_path: str, config: MaskedWordPieceDatasetConfig) -> IndexedDataset:
    ...
    def _build_sample_index(self, sequence_length: int, min_sentences_per_sample: int) -> numpy.ndarray:
        ...
        if not cache_hit and torch.distributed.get_rank() == 0:
            ...
            log_single_rank(logger, logging.INFO, f"> total number of samples: {sample_index.shape[0]}")
    ...
    def _create_masked_lm_predictions(
        ...
        candidate_ngrams = [[candidates[idx : idx + n] for n in ngram_nvals] for idx in range(len(candidates))]
        ...
        if self.config.masking_do_permutation:
            ...
            for candidate_idx in range(len(candidate_ngrams)):
                ...
                if any(map(lambda idx: idx in masked_indices or idx in permuted_indices, ngram_indices)):
                    continue
