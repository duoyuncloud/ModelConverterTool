...
class BlendedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if len(datasets) == 1:
            log_single_rank(logger, logging.WARNING, "Building a BlendedDataset for a single MegatronDataset")
    ...
    def _build_indices(self) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        if not path_to_cache or (not cache_hit and torch.distributed.get_rank() == 0):
            ...
            log_single_rank(logger, logging.INFO, "\tBuild and save the dataset and dataset sample indexes")
            t_beg = time.time()
        ...
        log_single_rank(logger, logging.INFO, f"\tLoad the dataset index from {path_to_dataset_index}")
        t_beg = time.time()
