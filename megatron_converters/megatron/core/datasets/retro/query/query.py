...
def query_embeddings(
    ...
    for chunk_id in range(min_chunk_id, max_chunk_id):
        ...
        # Filter row.
        filtered_row = [i for i in query_row if tuple(db_dataset.doc_tuples[i].tolist()) not in sample_doc_tuples]
...
def query_block_neighbors(
    ...
    # Sample map.
    sample_ids = sorted(list(set(chunk_id // n_chunks_per_sample for chunk_id in range(*block["range"]))))
    sample_map = {}
    for i in sample_ids:
