...
def build_block_db(
    ...
    chunk_db_valid = [item for partial_chunk_db in partial_chunk_dbs for item in partial_chunk_db[1]]
    chunk_db_invalid = [item for partial_chunk_db in partial_chunk_dbs for item in partial_chunk_db[2]]
    ...
    # Document offsets.
    doc_sizes = [(d, s) for partial_chunk_db in partial_chunk_dbs for d, s in partial_chunk_db[3].items()]
    doc_sizes.sort(key=lambda item: item[0])
    doc_offsets = np.cumsum([item[1] for item in doc_sizes]).astype("uint64")
    doc_offsets = np.stack((np.array([item[0] for item in doc_sizes], dtype="uint64"), doc_offsets), axis=1)
...
def update_chunk_counts(config: RetroPreprocessingConfig, indexed_dataset_infos: List[Dict]) -> None:
    ...
    for ds_index, ds_info in enumerate(indexed_dataset_infos):
        ...
        for db_path in tqdm(db_paths, "%d/%d, %s" % (ds_index, len(indexed_dataset_infos), ds_info["prefix"])):
            with h5py.File(db_path, "r") as f:
                ds_info["n_chunks"] += len(f["chunks_valid"])
                ds_info["n_chunks_invalid"] += len(f["chunks_invalid"])
                ds_info["n_chunks_train"] += (np.copy(f["chunks_valid"][:, 0]) < ds_info["n_docs_train"]).sum().item()

        ds_info["n_chunks_sampled"] = int(config.retro_index_ntrain * ds_info["ratio"] / data_ratio_sum)
        ...
        assert ds_info["n_chunks_sampled"] <= ds_info["n_chunks_train"], "n_sampled (%d) > n_train (%d)." % (
...
def merge_dbs(project_dir: str, indexed_dataset_infos: List[Dict], db_type: str) -> None:
    ...
    if not os.path.exists(db_path):
        ...
        merged_doc_offsets: np.ndarray = (
            None if n_docs_key is None else f.create_dataset("doc_offsets", (n_docs, 3), dtype="uint64")
        )
        ...
        for ds_idx, ds_info in enumerate(indexed_dataset_infos):
            ...
            individual_doc_offsets: np.ndarray = (
                None if n_docs_key is None else get_individual_doc_offsets(project_dir, ds_idx, ds_info)
            )
            ...
            if db_type == "valid":
                ...
                if n_docs_key is None:
                    ...
                    individual_doc_offsets = np.copy(individual_doc_offsets[ds_info["n_docs_train"] :])
                ...
                individual_doc_offsets = (
                    None if n_docs_key is None else np.copy(individual_doc_offsets[: ds_info[n_docs_key]])
                )
