...
class BERTMaskedWordPieceDataset(MaskedWordPieceDataset):
    ...
    def _key_config_attributes() -> List[str]:
        ...
        """
        return super(BERTMaskedWordPieceDataset, BERTMaskedWordPieceDataset)._key_config_attributes() + [
            "classification_head",
        ]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[int, numpy.ndarray]]:
        ...
        assignments = numpy.pad(assignments, (0, length_pads), constant_values=self.config.tokenizer.pad)

        # Get the padding mask
        mask_pads = numpy.ones(length_toks, dtype=numpy.int64)
        mask_pads = numpy.pad(mask_pads, (0, length_pads), constant_values=self.config.tokenizer.pad)
