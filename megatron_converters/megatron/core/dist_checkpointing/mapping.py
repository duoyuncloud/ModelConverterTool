...
class ShardedTensor(ShardedBase):
    ...
    def global_coordinates(self) -> Tuple[np.ndarray, ...]:
        if self.flattened_range is None:
            raise CheckpointingException(
                f"`global_coordinates` is undefined for" f" {self.__class__.__name__} without `flattened_range`"
            )
        ...
        global_coords = tuple(
            c + off for c, off in zip((0,) * self.prepend_axis_num + local_coords, self.global_offset)
        )
    ...
    def local_coordinates(self) -> Tuple[np.ndarray, ...]:
        if self.flattened_range is None:
            raise CheckpointingException(
                f"`local_coordinates` is undefined for" f" {self.__class__.__name__} without `flattened_range`"
            )
...
def apply_factory_merges(x1: StateDict, x2: ShardedStateDict, key: Tuple[str, ...] = ()) -> StateDict:
    ...
    if isinstance(x1, dict) and isinstance(x2, dict):
        ...
        if len(x1) != len(x2):
            err_msg = (
                f"Cannot merge two lists with different lengths ({len(x1)} and {len(x2)}, encountered at key {key})"
            )
        ...
        for k, v2 in x2.items():
            ...
            if k >= len(x1):
                raise ValueError(f"Dict key {k} out of bound for list of length {len(x1)} (encountered at level {key})")
            x1[k] = apply_factory_merges(x1[k], v2, key=key + (k,))
    else:
        raise ValueError(f"Duplicate non-dict and non-list values encountered: `{x1}` and `{x2} (at key {key})`")
    return x1
