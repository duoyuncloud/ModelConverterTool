...
# TODO: implement it as common torch strategy
def _save_common_dict(state_dict: StateDict, checkpoint_dir: Path, validate_consistency: bool = False):
    if torch.distributed.get_rank() == 0:
...
def _extract_and_save_sharded_objects(state_dict: StateDict, checkpoint_dir: Path, validate_consistency: bool = False):
    sharded_objects, state_dict = extract_matching_values(state_dict, lambda v: isinstance(v, ShardedObject))
    ...
    def chunk_offset(sharding):
        ...
        return tuple(
            chain(
                ...
                (
                    off // sh
                    for off, sh in zip(sharding.global_offset[sharding.prepend_axis_num :], sharding.local_shape)
                ),
            )
        )
...
def _validate_sharding_for_key_flattened(tensors_by_shard):
    ...
    if starts[0] != 0 or stops[-1] != np.product(local_shape) or not np.all(starts[1:] == stops[:-1]):
...
def _validate_objects_for_key(sharded_objects: List[ShardedObject]):
    ...
    unique_keys = [sh_obj.unique_key for _, sh_obj in sharded_objects if is_main_replica(sh_obj.replica_id)]
