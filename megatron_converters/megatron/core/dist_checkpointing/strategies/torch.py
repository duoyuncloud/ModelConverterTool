...
def sharded_tensor_to_torch_sharded_tensor(
    ...
    if has_flattened_range:
        ...
        local_shards = [
            Shard.from_tensor_and_offsets(sh_ten.data, list(sh_ten.global_offset), rank) for sh_ten in sh_tens
        ]
...
class MCoreLoadPlanner(DefaultLoadPlanner):
    ...
    def __init__(self, *args, shapes_validation_sharded_tensors: Iterable[ShardedTensor] = (), **kwargs) -> None:
...
class TorchDistSaveShardedStrategy(AsyncSaveShardedStrategy):
    ...
    def __init__(self, backend: str, version: int, keep_only_main_replica: bool = True, thread_count: int = 2):
...
default_strategies[StrategyAction.LOAD_SHARDED.value][("torch_dist", 1)] = TorchDistLoadShardedStrategy()
default_strategies[StrategyAction.SAVE_SHARDED.value][("torch_dist", 1)] = TorchDistSaveShardedStrategy("torch_dist", 1)
