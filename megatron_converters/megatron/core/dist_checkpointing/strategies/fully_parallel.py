...
class FullyParallelSaveStrategyWrapper(AsyncSaveShardedStrategy):
    ...
    def async_save(self, sharded_state_dict: ShardedStateDict, checkpoint_dir: Path):
        if not isinstance(self.base_strategy, AsyncSaveShardedStrategy):
            raise CheckpointingException(f"Cannot apply async_save to non-async base strategy {self.base_strategy}")
    ...
    def apply_saving_parallelization(self, sharded_state_dict: ShardedStateDict) -> None:
        ...
        if self.do_cache_distribution and self.cached_distribution is not None:
            logger.debug("Apply *cached* save parallelization")
            precomputed_distribution = self.cached_distribution
        else:
            logger.debug("Apply save parallelization")
...
class FullyParallelLoadStrategyWrapper(LoadShardedStrategy):
    ...
    def apply_loading_parallelization(self, sharded_state_dict: ShardedStateDict) -> Optional[SaveLoadDistribution]:
        ...
        """
        if self.do_cache_distribution and self.cached_distribution is not None:
            logger.debug("Apply *cached* load parallelization")
            precomputed_distribution = self.cached_distribution
        else:
            logger.debug("Apply load parallelization")
            precomputed_distribution = determine_main_replica_uniform_distribution(
    ...
    def exchange_loaded_tensors_gather_object(
        ...
        torch.distributed.all_gather_object(all_loaded_tensors_list, loaded_tensors, group=parallelization_group)
        ...
        if len(all_loaded_tensors) != sum(map(len, all_loaded_tensors_list)):
            ...
            if torch.distributed.get_rank() == 0:
                logger.error(f"{err_msg}. Shards ids by rank: {[lt.keys() for lt in all_loaded_tensors_list]}")
    ...
    def exchange_loaded_tensors_broadcast(
        ...
        for shard_id, rank in shard_to_saving_rank.items():
            ...
            torch.distributed.broadcast(local_ten, src=global_src_rank, group=parallelization_group, async_op=True)
        ...
        def fill_in_sharded_tensor(x):
            if isinstance(x, ShardedTensor):
                try:
                    ...
                    raise CheckpointingException(f"Missing loaded tensor shard: {_sharded_tensor_shard_id(x)}") from e
...
def determine_main_replica_uniform_distribution(
    ...
    if group_size <= 1:
        return
    local_shards = list(sh_base for sh_base in nested_values(sharded_state_dict) if isinstance(sh_base, ShardedTensor))
    ...
    shard_to_ranks = {k: v for k, v in shard_to_ranks.items() if k in shards_saved_by_this_parallelization_group}

    shard_to_saving_rank = distribute_shards_to_ranks(shard_to_ranks, shard_to_size, len(all_shards))

    return SaveLoadDistribution(shard_to_saving_rank, shards_saved_by_this_parallelization_group, shard_to_metadata)
