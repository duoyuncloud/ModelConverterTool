...
def check_param_hashes_across_dp_replicas(model: List[torch.nn.Module]) -> bool:
    ...
    for model_chunk_id, model_chunk in enumerate(model):
        for param_name, param in model_chunk.named_parameters():
            param_hash = torch.frombuffer(
                array.array("B", hashlib.sha1(param.data.to("cpu").float().numpy(force=True)).digest()),
                dtype=torch.uint8,
            )
    ...
    all_param_hashes = [
        torch.zeros_like(local_param_hashes) for _ in range(parallel_state.get_data_parallel_world_size())
    ]
...
def drain_embedding_wgrad_compute(config, embedding_activation_buffer, grad_output_buffer, weight):
    ...
    def wgrad_compute(all_gathered_input, grad_output, weight):
        ...
        grad_output, all_gathered_input = prepare_input_tensors_for_wgrad_compute(grad_output, all_gathered_input)

        if config.gradient_accumulation_fusion:
            if weight.main_grad.dtype == torch.float32:
                fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32(all_gathered_input, grad_output, weight.main_grad)
            elif weight.main_grad.dtype in (torch.float16, torch.bfloat16):
                fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16(all_gathered_input, grad_output, weight.main_grad)
            else:
...
class StragglerDetector:
    ...
    def _handler(self) -> None:
        ...
        if self.rank == 0:
            ...
            logger.info(f"Controller ready to recv " f"commands on port {self.port}. Current state {state}")
            while True:
                try:
    ...
    def _controller(self):
        ...
        try:
            if self.rank == 0:
                ...
                self.ctrlr = threading.Thread(target=self._handler, args=(), name="straggler", daemon=True)
                self.ctrlr.start()
        except Exception as err:
