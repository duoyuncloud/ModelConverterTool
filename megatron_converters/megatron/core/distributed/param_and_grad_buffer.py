...
class Bucket:
    ...
    def start_grad_sync(self):
        ...
        if self.ddp_config.use_distributed_optimizer:
            local_data_view = shard_buffer(self.grad_data, self.data_parallel_world_size)[self.data_parallel_rank]
...
class ParamAndGradBuffer:
    ...
    def __init__(
        ...
        for param in params[::-1]:
            ...
            if (
                bucket_size is not None and (data_end_index - bucket_data_start_index) >= bucket_size
        ...
        for param in params[::-1]:
            ...
            if self.param_data is not None:
                ...
                param.data = self._get(param.data.shape, data_start_index, buffer_type=BufferType.PARAM)
                assert old_param_data._base is None
                # Copy tensor values (from initialization or checkpoint).
                param.data.detach().copy_(old_param_data)
                del old_param_data

            param.main_grad = self._get(param.data.shape, data_start_index, buffer_type=BufferType.GRAD)
        ...
        if (
            ...
            logger.info(f"Number of buckets for gradient all-reduce / reduce-scatter: {len(self.buckets)}")
    ...
    def _set_bucket(
        ...
        bucketed_grad_data = self._get(torch.Size([end_index - start_index]), start_index, buffer_type=BufferType.GRAD)
        bucket = Bucket(
