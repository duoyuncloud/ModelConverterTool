...
def _allreduce_position_embedding_grads(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    if (
        ...
        model_module = model[0]
        grad = get_attr_wrapped_model(model_module, "language_model.embedding.position_embeddings.weight.main_grad")
...
def _allreduce_layernorm_grads(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    if parallel_state.get_tensor_model_parallel_world_size() > 1 and (config.sequence_parallel or config.qk_layernorm):
        grads = []
        for model_chunk in model:
            for name, param in get_attr_wrapped_model(model_chunk, "named_parameters")():
                if getattr(param, "sequence_parallel", False) or "q_layernorm" in name or "k_layernorm" in name:
