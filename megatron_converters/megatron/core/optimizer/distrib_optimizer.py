...
# For conversion scripts, use torch optimizers instead
...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def __init__(
        ...
        assert isinstance(optimizer, Adam), "Only Adam currently supported, due to checkpointing requirements."
        ...
        for buffer in self.buffers:
            ...
            self.per_bucket_numel.append(
                {(buffer.param_dtype, buffer.grad_dtype): [bucket.grad_data.numel() for bucket in buffer.buckets]}
            )
            self.per_bucket_numel_unpadded.append(
                {(buffer.param_dtype, buffer.grad_dtype): [bucket.numel_unpadded for bucket in buffer.buckets]}
            )
        ...
        ) = self._build_model_and_main_param_groups(self.gbuf_ranges, self.model_param_gbuf_map, self.opt_group_ranges)
        ...
        for gbuf_index, dtype, bucket_index, _, _ in self.pbuf_view_items:
            self.all_gather_handle_index_to_bucket_index_map.append((gbuf_index, dtype, bucket_index))
    ...
    def sharded_param_state_dp_zero(self, model_sharded_state_dict: ShardedStateDict, is_loading: bool = False):
    ...
    def sharded_param_state_fs_bucket_space(self, model_sharded_state_dict: ShardedStateDict, is_loading: bool = False):
    ...
    def _get_model_param_buffer_dp_views(self):
        ...
        for gbuf_index, buffer in enumerate(self.buffers):
            ...
            for bucket_index, bucket in enumerate(buffer.buckets):
                ...
                view_items_per_model_chunk.insert(0, (gbuf_index, dtype, bucket_index, bucket.param_data, buf_views))
