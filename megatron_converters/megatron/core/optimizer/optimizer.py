...
# import amp_C
# For conversion scripts, we don't need amp_C
amp_C = None
import torch

# from apex.multi_tensor_apply import multi_tensor_applier
# For conversion scripts, we don't need multi_tensor_applier
...
class Float16OptimizerWithFloat16Params(MixedPrecisionOptimizer):
    ...
    def sharded_state_dict(self, model_sharded_state_dict: ShardedStateDict, is_loading: bool = False):
        if is_loading:
...
class ChainedOptimizer(MegatronOptimizer):
    ...
    def sharded_state_dict(self, model_sharded_state_dict: ShardedStateDict, is_loading: bool = False, **kwargs):
        ...
        for optimizer_idx, optimizer in enumerate(self.chained_optimizers):
            optim_state_dict = optimizer.sharded_state_dict(model_sharded_state_dict, is_loading, **kwargs)
    ...
    def load_state_dict(self, state_dict):
        if len(self.chained_optimizers) != len(state_dict):
            raise RuntimeError(
                f"Expected {len(self.chained_optimizers)} entries" f" in state dict, but got {len(state_dict)}."
            )
    ...
    def disable_pre_hook(self):
        for optimizer in self.chained_optimizers:
            if not optimizer.config.use_distributed_optimizer or not optimizer.config.overlap_param_gather:
                raise ValueError(
