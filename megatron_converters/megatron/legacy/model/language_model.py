...
class Embedding(MegatronModule):
    ...
    def __init__(self, hidden_size, vocab_size, max_sequence_length, embedding_dropout_prob, config, num_tokentypes=0):
...
class TransformerLanguageModel(MegatronModule):
    ...
    def __init__(
        ...
        if self.add_encoder:
            self.encoder = ParallelTransformer(
                config,
                model_type=args.model_type if not args.retro_add_retriever else ModelType.retro_decoder,
    ...
    def forward(
        ...
        if self.add_retriever and self.pre_process:
            retriever_input = self.embedding(retriever_input_ids, retriever_position_ids, tokentype_ids=tokentype_ids)
        else:
