...
class SwitchMLP(MegatronModule):
    ...
    def gather_indices(self, local_indices):
        ...
        output = torch.empty(dim_size, dtype=local_indices.dtype, device=torch.cuda.current_device())
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, config, layer_number, attention_type=AttnType.self_attn, attn_mask_type=AttnMaskType.padding):
        ...
        self.use_flash_attn = (
            args.use_flash_attn and attention_type == AttnType.self_attn and self.attn_mask_type == AttnMaskType.causal
        )
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def default_decoder_cross_attention(
        ...
        with self.bias_dropout_add_exec_handler():
            norm_input = bias_dropout_add_func(attention_output, attention_bias, residual, self.hidden_dropout)
    ...
    def retro_encoder_cross_attention(self, retriever_output, norm_input, norm_output, bias_dropout_add_func):
        ...
        chunked_outputs = norm_output.reshape(self.retro_retrieved_length, -1, self.retro_num_neighbors, d)
    ...
    def retro_decoder_cross_attention(
        ...
        padded_chunked_output = padded_chunks.reshape(l, self.retro_chunk_length, bs, d).permute(1, 2, 0, 3)
        padded_chunked_output = padded_chunked_output.reshape(self.retro_chunk_length, bs * l, d).contiguous()
        ...
        with torch.enable_grad():
            ...
            norm_input = norm_input.reshape(self.retro_chunk_length, bs, l, d).permute(2, 0, 1, 3)  # [l, m, bs, d]
            norm_input = norm_input.reshape(self.retro_chunk_length * l, bs, d)
            norm_input = torch.nn.functional.pad(norm_input, (0, 0, 0, 0, pad, 0), "constant", 0)[:ns]  # [ns, b, d]
    ...
    def forward(
        ...
        attention_output, attention_bias = self.self_attention(
            norm_output, attention_mask, inference_params=inference_params, rotary_pos_emb=rotary_pos_emb
        )
        ...
        if self.drop_path is None:
            ...
            out = torch.nn.functional.dropout(mlp_output, p=self.hidden_dropout, training=self.training)
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(
        ...
        self.distribute_saved_activations = config.distribute_saved_activations and not config.sequence_parallel
        ...
        if args.retro_add_retriever:
            ...
            assert args.transformer_impl == "local", "Transformer engine does not support Retro layers."

        def build_layer(layer_number):
            if args.transformer_impl == "local":
                current_layer_type = _get_layer_type(model_type, layer_type, self.retro_layer_numbers, layer_number)
            ...
            if model_type == ModelType.retro_encoder:
                for layer in self.layers:
                    if layer.self_attention.use_flash_attn:
                        ...
                        layer.self_attention.core_attention.attention_dropout.p = args.retro_encoder_attention_dropout
                ...
                if l < self.recompute_num_layers:
                    ...
                    if self.transformer_impl == "transformer_engine":
                        hidden_states = custom(l, l + 1)(
                            hidden_states, attention_mask, encoder_output, enc_dec_attn_mask, **te_forward_kwargs
                        )
                    else:
                        hidden_states = custom(l, l + 1)(
                            hidden_states,
