...
def get_student_backbone_and_num_features(config, pre_process=True, post_process=True):
    ...
    if args.vision_backbone_type == "vit":
        student = VitBackbone(
            config, pre_process=pre_process, post_process=post_process, drop_path_rate=0.1, single_token_output=True
        )
...
def get_teacher_backbone_and_num_features(config, pre_process=True, post_process=True):
    ...
    if args.vision_backbone_type == "vit":
        teacher = VitBackbone(config, pre_process=pre_process, post_process=post_process, single_token_output=True)
        num_features = args.hidden_size
    elif args.vision_backbone_type == "mit":
...
class DINOPretrainModel(MegatronModule):
    def __init__(self, config, pre_process=True, post_process=True):
        ...
        student_backbone, num_features = get_student_backbone_and_num_features(config, pre_process, post_process)
        ...
        teacher_backbone, num_features = get_teacher_backbone_and_num_features(config, pre_process, post_process)
