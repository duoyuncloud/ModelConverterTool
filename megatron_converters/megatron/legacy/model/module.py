...
class MegatronModule(torch.nn.Module):
    ...
    def initialize_word_embeddings(self):
        ...
        if not self.share_embeddings_and_output_weights:
            raise Exception(
                "initialize_word_embeddings() was called but " "share_embeddings_and_output_weights is false"
            )
        ...
        if mpu.is_pipeline_last_stage() and not self.pre_process:
            ...
            self.word_embeddings = tensor_parallel.VocabParallelEmbedding(
                args.padded_vocab_size, self.config.hidden_size, config=self.config, init_method=self.config.init_method
            )
