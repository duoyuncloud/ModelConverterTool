...
def model_provider_distil(pre_process=True, post_process=True) -> DistilModel:
    """Builds the model.
    ...
    if args.distillation:
        ...
        if teacher_transformer_config.use_mla:
            teacher_transformer_layer_spec = get_gpt_layer_with_mla_spec(
                teacher_transformer_config.num_experts, teacher_transformer_config.moe_grouped_gemm
            )
        else:
        ...
        raise ValueError("must distillation in the config, set --distillation")
...
if __name__ == "__main__":
    ...
    if args.distillation:
        model, optimizer, opt_param_scheduler = setup_model_and_optimizer(
            model_provider_distil, model_type=ModelType.encoder_or_decoder
        )
    else:
        model, optimizer, opt_param_scheduler = setup_model_and_optimizer(
    ...
    if args.use_mla:
        for layer_idx in range(args.num_layers):
            ...
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.q_b_proj.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.linear_q_up.weight"
            ]
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.q_a_proj.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.linear_q_down.weight"
            ]
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.kv_b_proj.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.linear_kv_up.weight"
            ]
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.kv_a_proj_with_mqa.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.linear_kv_down.weight"
            ]
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.q_a_layernorm.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.q_layernorm.weight"
            ]
            state_dict_hf[f"model.layers.{layer_idx}.self_attn.kv_a_layernorm.weight"] = state_dict_megatron[
                f"decoder.layers.{layer_idx}.self_attention.kv_layernorm.weight"
            ]
        ...
        for layer_idx in range(args.num_layers):
            ...
            qkv_proj_split = torch.split(
                qkv_proj, split_size_or_sections=(args.hidden_size // args.num_attention_heads), dim=0
            )
            ...
            if args.add_qkv_bias:
                qkv_bias = state_dict_megatron[f"decoder.layers.{layer_idx}.self_attention.linear_qkv.bias"]
                qkv_bias_split = torch.split(
                    qkv_bias, split_size_or_sections=(args.hidden_size // args.num_attention_heads), dim=0
                )
                ...
                q_bias = torch.cat(q_bias_list, dim=0)
                k_bias = torch.cat(k_bias_list, dim=0)
                v_bias = torch.cat(v_bias_list, dim=0)
                state_dict_hf[f"model.layers.{layer_idx}.self_attn.q_proj.bias"] = q_bias
                state_dict_hf[f"model.layers.{layer_idx}.self_attn.k_proj.bias"] = k_bias
                state_dict_hf[f"model.layers.{layer_idx}.self_attn.v_proj.bias"] = v_bias
...
def convert_minicpm_megatron_to_hf(
    checkpoint_path: str,
    output_path: str,
    num_layer: int,
    tp_size: int,
    ...
    **kwargs,
    ...
    """
    Wrapper function for MiniCPM Megatron to HF conversion using distributed checkpointing
    ...
    print(f"Converting MiniCPM Megatron to HF: {checkpoint_path} -> {output_path}")
