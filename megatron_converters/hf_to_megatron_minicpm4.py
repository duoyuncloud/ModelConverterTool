...
parser.add_argument("--num_query_heads", type=int, default=24)
parser.add_argument("--dense_layer_ids", type=str, default="")
parser.add_argument("--use_mla", action="store_true")
args = parser.parse_args()

if args.pp_size != 1:
    output_path = args.save_dir + "/" + f"mp_rank_0{args.tp_rank}_00{args.pp_rank}/"
else:
    output_path = args.save_dir + "/" + f"mp_rank_0{args.tp_rank}"
os.makedirs(output_path, exist_ok=True)

cpm_model_dict = torch.load(args.load)
# llama_model = AutoModelForCausalLM.from_pretrained(args.load, torch_dtype=torch.bfloat16, trust_remote_code=True)
...
assert embedding.shape[0] % args.tp_size == 0
if args.pp_rank == 0:
    megatron_model_dict["embedding.word_embeddings.weight"] = torch.split(
        embedding, embedding.shape[0] // args.tp_size, dim=0
    )[args.tp_rank]
if args.pp_rank == args.pp_size - 1:
...
assert args.num_layer % args.pp_size == 0

layer_num_per_pp = args.num_layer // args.pp_size
num_query_heads_per_group = args.num_query_heads // args.num_kv_heads

args.dense_layer_ids = list(map(int, args.dense_layer_ids.split(",")))

if args.use_mla:
    for layer_idx in range(args.pp_rank * layer_num_per_pp, (args.pp_rank + 1) * layer_num_per_pp):
        print(f"process layer {layer_idx}")
        layer_idx_abs = layer_idx - args.pp_rank * layer_num_per_pp
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx}.self_attention.linear_q_up.weight"] = torch.split(
            q_b_proj_weight, dim=0, split_size_or_sections=q_b_proj_weight.shape[0] // args.tp_size
        )[args.tp_rank].clone()
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx}.self_attention.linear_q_down.weight"] = torch.split(
            q_a_proj_weight, dim=0, split_size_or_sections=q_a_proj_weight.shape[0] // args.tp_size
        )[args.tp_rank].clone()
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx}.self_attention.linear_kv_up.weight"] = torch.split(
            kv_b_proj_weight, dim=0, split_size_or_sections=kv_b_proj_weight.shape[0] // args.tp_size
        )[args.tp_rank].clone()
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx}.self_attention.linear_kv_down.weight"] = torch.split(
            kv_a_proj_weight, dim=0, split_size_or_sections=kv_a_proj_weight.shape[0] // args.tp_size
        )[args.tp_rank].clone()
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx}.self_attention.linear_proj.weight"] = torch.split(
            o_weight, dim=1, split_size_or_sections=o_weight.shape[1] // args.tp_size
        )[args.tp_rank].clone()
        ...
        w0_weight_cu_rank = torch.split(w0_weight, w0_weight.shape[0] // args.tp_size, dim=0)[args.tp_rank]
        w1_weight_cu_rank = torch.split(w1_weight, w1_weight.shape[0] // args.tp_size, dim=0)[args.tp_rank]
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.linear_fc2.weight"] = torch.split(
            w2_weight, w2_weight.shape[1] // args.tp_size, dim=1
        )[args.tp_rank].clone()
    ...
    for layer_idx in range(args.pp_rank * layer_num_per_pp, (args.pp_rank + 1) * layer_num_per_pp):
        ...
        qkv_weight_list = []
        for i in range(args.num_kv_heads):
            q_group_weight = []
        ...
        qkv_weight_tp = torch.split(qkv_weight, qkv_weight.shape[0] // args.tp_size, dim=0)
        ...
        megatron_model_dict[f"decoder.layers.{layer_idx_abs}.self_attention.linear_proj.weight"] = torch.split(
            o_weight, o_weight.shape[1] // args.tp_size, dim=1
        )[args.tp_rank]
        ...
        if layer_idx in args.dense_layer_ids:
            w0_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.gate_proj.weight"]
            w1_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.up_proj.weight"]
            w2_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.down_proj.weight"]
            w0_weight_cu_rank = torch.split(w0_weight, w0_weight.shape[0] // args.tp_size, dim=0)[args.tp_rank]
            ...
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_router.weight"] = cpm_model_dict[
                f"model.layers.{layer_idx}.mlp.moe_router.weight"
            ]
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.router_norm.weight"] = cpm_model_dict[
                f"model.layers.{layer_idx}.mlp.router_norm.weight"
            ]
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_w_in.weight"] = cpm_model_dict[
                f"model.layers.{layer_idx}.mlp.moe_w_in.weight"
            ]
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_w_out.weight"] = cpm_model_dict[
                f"model.layers.{layer_idx}.mlp.moe_w_out.weight"
            ]
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_router._extra_state"] = None
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_w_in._extra_state"] = None
            megatron_model_dict[f"decoder.layers.{layer_idx_abs}.mlp.moe_w_out._extra_state"] = None
...
def convert_hf_to_megatron_minicpm4(
    checkpoint_path: str,
    output_path: str,
    num_layer: int,
    tp_size: int,
    ...
    **kwargs,
    ...
    """
    print(f"Converting HF to MiniCPM-4 Megatron: {checkpoint_path} -> {output_path}")
