...
def convert_hf_to_megatron_minicpm4_main(
    load_path: str,
    num_layer: int = 52,
    tp_size: int = 1,
    tp_rank: int = 0,
    ...
    """
    Main conversion function for HF to MiniCPM-4 Megatron conversion
    ...
    Args:
        ...
        dense_layer_ids: Comma-separated list of dense layer IDs
    ...
    dense_layer_ids_list = list(map(int, dense_layer_ids.split(","))) if dense_layer_ids else []
    ...
    if use_mla:
        ...
        for layer_idx in range(pp_rank * layer_num_per_pp, (pp_rank + 1) * layer_num_per_pp):
            ...
            if layer_idx in dense_layer_ids_list:
                w0_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.gate_proj.weight"]
                w1_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.up_proj.weight"]
                w2_weight = cpm_model_dict[f"model.layers.{layer_idx}.mlp.down_proj.weight"]
                w0_weight_cu_rank = torch.split(w0_weight, w0_weight.shape[0] // tp_size, dim=0)[tp_rank]
...
def main():
    ...
    parser.add_argument("--dense_layer_ids", type=str, default="", help="Comma-separated list of dense layer IDs")
    ...
    convert_hf_to_megatron_minicpm4_main(
        ...
        dense_layer_ids=args.dense_layer_ids,
