...
class ModelBase:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, *, is_big_endian: bool = False,
        ...
        if self.ftype == gguf.LlamaFileType.GUESSED:
            ...
            # 修复：兼容safetensors懒加载切片对象
            dtype = getattr(first_tensor, 'dtype', None)
            if dtype is None and hasattr(first_tensor, 'get_tensor'):
                try:
                    dtype = first_tensor.get_tensor().dtype
                except Exception:
                    dtype = None
            if dtype == torch.float16:
                logger.info(f"choosing --outtype f16 from first tensor type ({dtype})")
                self.ftype = gguf.LlamaFileType.MOSTLY_F16
            else:
                logger.info(f"choosing --outtype bf16 from first tensor type ({dtype})")
    ...
    def prepare_tensors(self):
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            # 强化：递归get_tensor直到为torch.Tensor
            max_unwrap = 5
            unwrap_count = 0
            while not hasattr(data_torch, 'dtype') and hasattr(data_torch, 'get_tensor') and unwrap_count < max_unwrap:
                try:
                    data_torch = data_torch.get_tensor()
                    unwrap_count += 1
                except Exception:
                    break

            if not hasattr(data_torch, 'dtype'):
                logger.error(f"[prepare_tensors] Could not unwrap tensor for {name}, got type {type(data_torch)}")
                continue
...
class TextModel(ModelBase):
    ...
    def get_vocab_base_pre(self, tokenizer) -> str:
        ...
        chktxt = '\n \n\n \n\n\n \t \t\t \t\n  \n   \n    \n     \n🚀 (normal) 😶\u200d🌫️ (multiple emojis concatenated) ✅ 🦙🦙 3 33 333 3333 33333 333333 3333333 33333333 3.3 3..3 3...3 កាន់តែពិសេសអាច😁 ?我想在apple工作1314151天～ ------=======殊不知在 Български \'\'\'\'\'\'```````""""......!!!!!!?????? I\'ve been \'told he\'s there, \'RE you sure? \'M not sure I\'ll make it, \'D you like some tea? We\'Ve a\'lL'
        ...
        if chkhsh == "35d91631860c815f952d711435f48d356ebac988362536bed955d43bfa436e34":
            ...
            res = "starcoder"
        if chkhsh == "3ce83efda5659b07b1ad37ca9702a658b3b88729b217a74792e9f8d43e479d21":
    ...
    def _set_vocab_rwkv_world(self):
        ...
        tokens: list[bytes] = [' '.encode("utf-8")]
...
class OPTModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        head_count_kv = hparams.get("num_key_value_heads", head_count)
        ctx_length = hparams.get("max_position_embeddings", 2048)
        self.gguf_writer.add_context_length(ctx_length)
        self.gguf_writer.add_embedding_length(hparams["hidden_size"])
        self.gguf_writer.add_block_count(block_count)
        ...
        self.gguf_writer.add_file_type(self.ftype)
        # OPT 没有 rope_theta，直接用 hidden_size // num_attention_heads
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        mapping = {
            ...
            "model.decoder.final_layer_norm.bias": "output_norm.bias",
            "lm_head.weight": "output.weight",
        }
        if name.startswith("model.decoder.layers."):
            parts = name.split(".")
            layer_id = int(parts[3])
            subname = ".".join(parts[4:])
            if subname == "self_attn.q_proj.weight":
                new_name = f"layers.{layer_id}.attn_q.weight"
            elif subname == "self_attn.k_proj.weight":
                new_name = f"layers.{layer_id}.attn_k.weight"
            elif subname == "self_attn.v_proj.weight":
                new_name = f"layers.{layer_id}.attn_v.weight"
            elif subname == "self_attn.out_proj.weight":
                new_name = f"layers.{layer_id}.attn_out.weight"
            elif subname == "self_attn_layer_norm.weight":
                new_name = f"layers.{layer_id}.attn_norm.weight"
            elif subname == "self_attn_layer_norm.bias":
                new_name = f"layers.{layer_id}.attn_norm.bias"
            elif subname == "fc1.weight":
                new_name = f"layers.{layer_id}.ffn_up.weight"
            elif subname == "fc2.weight":
                new_name = f"layers.{layer_id}.ffn_down.weight"
            elif subname == "final_layer_norm.weight":
                new_name = f"layers.{layer_id}.ffn_norm.weight"
            elif subname == "final_layer_norm.bias":
                new_name = f"layers.{layer_id}.ffn_norm.bias"
            else:
                new_name = name
            return [(new_name, data_torch)]
        if name in mapping:
            return [(mapping[name], data_torch)]
        return [(name, data_torch)]


def get_model_architecture(hparams, model_type):
    # 兼容 OPT、Llama、GPT2 等常见模型
    if "architectures" in hparams and hparams["architectures"]:
        return hparams["architectures"][0]
    if "model_type" in hparams:
        mt = hparams["model_type"].lower()
        if mt == "opt":
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("hf-to-gguf-debug")
    logger.info(f"[DEBUG] Script started with argv: {sys.argv}")
    import argparse
    parser = argparse.ArgumentParser(description="Convert HuggingFace model to GGUF format.")
    parser.add_argument("model", type=str, help="Model path or HuggingFace Hub id")
    parser.add_argument("outfile", type=str, help="Output GGUF file path")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    args = parser.parse_args()
    if args.verbose:
        logger.setLevel(logging.DEBUG)
    try:
        logger.info(f"[DEBUG] Loading model config: {args.model}")
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(args.model, trust_remote_code=True)
        logger.info(f"[DEBUG] Model config loaded: {config}")
        arch = config.architectures[0] if hasattr(config, 'architectures') and config.architectures else None
        logger.info(f"[DEBUG] Model architecture: {arch}")
        from pathlib import Path
        import gguf
        # 判断本地/远程
        if os.path.isdir(args.model):
            ...
            logger.info(f"[DEBUG] Downloading model from HuggingFace Hub: {args.model}")
            from transformers.utils.hub import snapshot_download
            tmpdir = tempfile.mkdtemp(prefix="hf-gguf-")
            model_dir = Path(snapshot_download(args.model, cache_dir=tmpdir, local_files_only=False, resume_download=True))
            logger.info(f"[DEBUG] Model downloaded to: {model_dir}")
        ...
        try:
            ...
            logger.error(f"[DEBUG] Could not find GGUF model class for arch {arch}: {e}")
            sys.exit(1)
        logger.info(f"[DEBUG] Using GGUF model class: {ModelClass}")
        # 实例化 GGUF 转换类
        model = ModelClass(model_dir, gguf.LlamaFileType.GUESSED, output_path)
        logger.info(f"[DEBUG] Instantiated GGUF model class.")
        # 写入 GGUF 文件
        model.write()
        logger.info(f"[DEBUG] GGUF file written to: {output_path}")
    except Exception as e:
